...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.head_img = nn.Sequential(
            ...
            nn.Linear(
                self.num_features,
                int(img_size / 4 * img_size / 4 * self.mask_ratio) * embed_dim,
            ),
        )
    ...
    def forward_features(self, x, num=None):
        ...
        x = self.pos_drop(x)  # torch.Size([25, 3136, 96])

        # out = self.random_masking(x, self.mask_ratio)
        # x, x_masked = out[0], out[1]
    ...
    def forward(self, img, num, mask, aux={}):
        ...
        loss_num = nn.functional.l1_loss(num, num_hat)
    ...
    def random_masking(self, x, mask_ratio):
        ...
        fill_value = nn.Parameter(
            torch.zeros(ids_keep.shape).unsqueeze(-1).repeat(1, 1, D)
        )
        trunc_normal_(fill_value, mean=0, std=0.02)
        x = torch.scatter(
            x,
            dim=1,
            index=ids_keep.unsqueeze(-1).repeat(1, 1, D),
            src=trunc_normal_(fill_value, std=0.2).to(x.device),
        )
...
if __name__ == "__main__":
    d = torch.rand((60, 17, 224, 224)).cuda()
