...
# classes
fds_config = dict(feature_dim=512, start_update=0,
...
class ViT(nn.Module):
    ...
    def forward(self, img, num, other=None):
        # 把num扩充到20维
        ...
        if not other is None:
            if other["epoch"] >= fds_config['start_smooth']:
                x = self.FDS.smooth(x, other["label"], other["epoch"])

        x = self.to_latent(x)
        return torch.squeeze(self.mlp_head(x)), x
